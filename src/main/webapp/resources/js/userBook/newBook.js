...
$(function(){
	...
	//목록버튼 누르면 실행
	$("#newBookListBtn").click(function() {
		$("#searchForm").attr("action","/newBook/newBookPage");
		...
		success: function(res) {
			...
			$.each(reList, function(i, e) {
				...
				viewList += "<td  valign='top' class='hover' onclick='goDetail(" + e.gdNo + ")'>" 
								...
								+"</ul>"
							+"</td>";
				viewList += "<td class='hover'>" + "<button id='cartBtn' onclick='addCart(${vo.gdNo})'>장바구니</button></td>";
